return {
	{
		...
		config = function()
			...
			local get_namespace = function(type)
				...
				local files = vim.fn.systemlist(string.format("rg --files --type csproj %s", vim.fn.getcwd()))
				...
				for _, file in ipairs(files) do
					...
					if current:find(dir, 1, true) then
						local n = vim.fn.fnamemodify(dir, ":t") .. current:sub(#dir + 1)
				...
				if namespace == nil then
					print("no .csproj matching current file buffer found")
					return
				end
				...
				if type:find("interface", 1, true) then
					class = "public " .. type .. " I" .. vim.fn.fnamemodify(vim.fn.expand("%:t"), ":r")
				else
					class = "public " .. type .. " " .. vim.fn.fnamemodify(vim.fn.expand("%:t"), ":r")
				end
